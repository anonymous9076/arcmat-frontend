...
const appendMoodboardData = async (worksheet, moodboard, project, products = [], startRow) => {
    ...
    const headers = ['Product Image', 'Name', 'Spec Status', 'Project Name', 'Brand', 'Unique Code', 'Quantity', 'Unit Price', 'Total Cost'];
    ...
    for (let i = 0; i < allOverviewItems.length; i++) {
        ...
        row.getCell(6).value = isProduct ? (data?.product_unique_id || (typeof data?.productId === 'object' ? data?.productId?.product_unique_id : '') || '') : '—';
...
export const exportProjectToExcel = async (project, moodboards = [], options = {}) => {
    ...
    if (!returnBuffer) toast.loading('Preparing project-wide excel export...', { id: 'export-toast' });
    ...
    try {
        ...
        if (!returnBuffer) toast.error('Failed to generate professional report', { id: 'export-toast' });
        return null;
    }
};
...
export const exportProjectToZip = async (project, moodboards = []) => {
    ...
    const toastId = toast.loading('Creating project ZIP bundle...', { id: 'zip-export' });

    try {
        const zip = new JSZip();
        const projectName = project?.projectName || 'project';
        const sanitizedName = projectName.replace(/\s+/g, '-');

        // 1. Generate Excel Report
        toast.loading('Generating Excel report...', { id: toastId });
        const excelBuffer = await exportProjectToExcel(project, moodboards, { returnBuffer: true });
        if (excelBuffer) {
            zip.file(`${sanitizedName}-specification-report.xlsx`, excelBuffer);
        }

        // 2. Fetch all renders
        const allRenders = moodboards.flatMap(s => 
            (s?.customPhotos || []).filter(p => (p.tags || []).includes('Render'))
        );

        if (allRenders.length > 0) {
            toast.loading(`Fetching ${allRenders.length} render images...`, { id: toastId });
            const renderFolder = zip.folder('renders');
            
            await Promise.all(allRenders.map(async (render, index) => {
                try {
                    const response = await fetch(render.previewUrl);
                    const blob = await response.blob();
                    const extension = render.previewUrl.split('.').pop().split('?')[0].toLowerCase() || 'jpg';
                    const fileName = `${sanitizedName}-${render.title || `render-${index + 1}`}.${extension}`;
                    renderFolder.file(fileName, blob);
                } catch (e) {
                    console.error('Failed to fetch render image:', render.previewUrl, e);
                }
            }));
        }
        ...
        const link = document.createElement('a');
        link.href = url;
        link.download = `${sanitizedName}-export.zip`;
        link.click();
        URL.revokeObjectURL(url);

        toast.success('Project ZIP exported successfully!', { id: toastId });
    } catch (error) {
        console.error('ZIP export failed:', error);
        toast.error('Failed to create project ZIP', { id: toastId });
    }
};
